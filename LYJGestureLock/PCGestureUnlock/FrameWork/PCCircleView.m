...
//        self.defaultSetting = delegate.defaultSetting;
//        self.setting = delegate.setting;
//        self.userInfo = self.setting.userInfo;
        // 解锁视图准备
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    [self.subviews enumerateObjectsUsingBlock:^(PCCircle *circle, NSUInteger idx, BOOL *stop) {
        if (CGRectContainsPoint(circle.frame, point)) {
            [circle setGestState:CircleStateSelected];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    [self.circleSet enumerateObjectsUsingBlock:^(PCCircle *circle, NSUInteger idx, BOOL *stop) {
        ...
        if (self.type != CircleViewTypeSetting) {
            [circle setGestState:CircleStateLastOneSelected];
        }
    }];
...
{
    return [(PCCircle *)[self.circleSet firstObject] gestState];
}
...
{
    [[self.circleSet lastObject] setGestState:state];
}
...
- (NSString *)getGestureResultFromCircleSet:(NSMutableArray *)circleSet
{
    ...
    for (PCCircle *circle in circleSet) {
        // 遍历取tag拼字符串
        [gesture appendFormat:@"%@", @(circle.tag - 1)];
    }
